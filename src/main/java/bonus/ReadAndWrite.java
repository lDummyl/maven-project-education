...
//    static Map<String, Report> averageSalary = new ConcurrentHashMap<>();
    ...
    static {
        averageSalary.put(petya, new Report());
    }
    ...
    @SneakyThrows
//    @Synchronized // TODO: 12/13/19 если синхронизировать эту операцию то профита по времени не будет. При этом если убрать то не изменится и результат.
    ...
    private static void syncWrite(PayCheck p) {
        Report report = averageSalary.get(p.name);
        report.payTimes++;
        report.summ += p.summ;
